...
{
	double dt = (2*PI)/(ensemble.OmegaRF*StepsPrPeriode);//1/(ensemble.OmegaRF/2/PI)/StepsPrPeriode;
	...
	for(int t=1; t <= TimeSteps ; t++)
	{
		...
		{
			// This is dim X
			double Ftot = Ftrap(ensemble, N, t-1, 0, Vrf, Vend,dt) + ((double) ForceX[N],dt);
			...
			// This is dim Y
			Ftot = Ftrap(ensemble, N, t-1, 1, Vrf, Vend,dt) + ((double) ForceY[N]);
			...
			// This is dim Z
			Ftot = Ftrap(ensemble, N, t-1, 2, Vrf, Vend,dt) + ((double) ForceZ[N]);
		...
		*/
		...
		if (((t) % ((int) (StepsPrPeriode))) == 0)
		{
			...
			*/
						...
						// Calculating the root mean square of termal vel's.
			double tau = (2*PI) / ensemble.OmegaRF; 
...
void TauPeriodeCudaLeFrogintegrator(FastEnsemble & ensemble, int TimeSteps, int StartHistograms, double Vrf, double Vend)
{
	...
	double dt = 1/(ensemble.OmegaRF/2/PI)/StepsPrPeriode;
	// allocate memory to the histogram
	ensemble.InitialiseHistogram();
	ensemble.InitialiseVelocityHistogram();
	ensemble.InitialiseCountHistogram();
	...
	for(int t=1; t <= TimeSteps ; t++)
	{
		...
		{
			// This is dim X
			double Ftot = Ftrap(ensemble, N, t-1, 0, Vrf, Vend,dt) + ((double) ForceX[N]);
			TempPos[N][0]  = ensemble.ions[N].Pos[0] + dt*dt*Ftot/ensemble.ions[N].m/2 + dt*ensemble.ions[N].Vel[0];
			TempVel[N][0]  = ensemble.ions[N].Vel[0] + dt/ensemble.ions[N].m*Ftot;
...
void TwoIonDynamicTemperatureLeFrogintegrator(FastEnsemble & ensemble, int TimeSteps, int StartHistograms, double Vrf, double Vend,double StepsPrPeriode) 
{
	...
	double Total_V_y_rms = 0.0;
	// 
	ofstream TemperaturefileOne ("TemperatureDataIonOne.txt");
	ofstream TemperaturefileTwo ("TemperatureDataIonTwo.txt");
	ofstream TemperaturefileBoth ("TemperatureDataIons.txt");
	    ...
	    if (((int)(t+ LowestPartOfPeriode-1) % ((int) (StepsPrPeriode))) == 0 && ((t) % ((int) (StepsPrPeriode))) != 0)  // for 105	
		{
			...
			for(int N = 0; N < ensemble.GetNumberOfIons(); N++)
			{
				...
				Periodefile << t								<< ",  " <<
								...
								ensemble.ions[N].Vel[2]			<< ",  " <<
								ensemble.ions[N].Velocity()		<< ",  " <<
								(pow(TotalV,2) * ensemble.ions[N].m)/(3*Kb)	<< ",  " <<
								endl;
				}   
			}
			}
